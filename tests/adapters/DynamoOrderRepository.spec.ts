...
import { DynamoOrderRepository } from '../../src/adapters/DynamoOrderRepository'
...
describe('DynamoOrderRepository', () => {
  ...
  const order = { id: '1', customerName: 'A', amount: 10, status: 'PENDING', createdAt: 't' } as any
    ;(ddbDocClient.put as jest.Mock).mockResolvedValue({})
  ...
  it('findAll should call scan and return items', async () => {
    const repo = new DynamoOrderRepository()
    ;(ddbDocClient.scan as jest.Mock).mockResolvedValue({ Items: [{ id: '1' }] })
  ...
  it('findById should call get and return item or null', async () => {
    const repo = new DynamoOrderRepository()
    ;(ddbDocClient.get as jest.Mock).mockResolvedValue({ Item: { id: '1' } })
    ...
    ;(ddbDocClient.get as jest.Mock).mockResolvedValue({})
  ...
  it('update should call update and return Attributes', async () => {
    const repo = new DynamoOrderRepository()
    ;(ddbDocClient.update as jest.Mock).mockResolvedValue({ Attributes: { id: '1', customerName: 'New' } })
  ...
  it('delete should call delete', async () => {
    const repo = new DynamoOrderRepository()
    ;(ddbDocClient.delete as jest.Mock).mockResolvedValue({})
